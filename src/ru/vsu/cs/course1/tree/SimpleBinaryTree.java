...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
   ...
   /* public <T extends Comparable<T>> double solution(){
        Comparator<Comparable<? extends Comparable<?>>> c = Comparable::compareTo;
        double min = 0;
        for (T i : BinaryTreeAlgorithms.preOrderValues(root)) {
            if(c.compare(min, i) > 0){
min = (Double) i;
            }
    ...
    private<T extends Comparable<T>>  int comparing(T i, T j){
        Comparator<T> c = Comparable::compareTo;
        int a;
        if(c.compare(i,j) > 0){
            a = 1;
        }else if(c.compare(i,j) == 0){
            a = 0;
        }else{
            a = -1;
        }
